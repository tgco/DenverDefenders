...
public class MovesOnSetPathAttribute extends Attribute {
	...
	public void update(GameObject myObject, Array<GameObject> allObjects) {
		//get next point
		float tolerance = Float.parseFloat(myObject.getAttributeData().get(this).get(0));
		int index = Integer.parseInt(myObject.getAttributeData().get(this).get(1));
		float[] targetPoint = new float[] {Float.parseFloat(myObject.getAttributeData().get(this).get((index*2)+2)),Float.parseFloat(myObject.getAttributeData().get(this).get((index*2)+3))};
		if(distance(myObject.getX(),myObject.getY(),targetPoint[0],targetPoint[1]) < tolerance){//if close enough to next point
			index++;
			if(index*2+3 >= myObject.getAttributeData().get(this).size) index = 0;
			myObject.getAttributeData().get(this).set(1,index+"");
			float[] nextPoint = new float[] {Float.parseFloat(myObject.getAttributeData().get(this).get((index*2)+2)),Float.parseFloat(myObject.getAttributeData().get(this).get((index*2)+3))};
	...
	public void setup(GameObject myObject){//tolderance, current goal index, list of points
		String[] points = myObject.getAttributeData().get(this).get(1).split(",");
		myObject.getAttributeData().get(this).set(1,"0");//this is the "current point", 0,1,2,3,4,5,...
		//parse points into the array
		for(int i = 0;i<points.length;i++)
			myObject.getAttributeData().get(this).add(points[i]);
		//get first point
